...
def print_formatted(number):
    try:
        ...
        print('An Error on input, type a integer value between 1~99.')
    else:
        for i in range(0, number):
            size = len((bin(number)[1:]))
            print('{}{}{}{}'.format(str(i+1).rjust(size-1), oct(i+1)[2:].rjust(size), hex(i+1)[2:].upper().rjust(size), bin(i+1)[2:].rjust(size)))
